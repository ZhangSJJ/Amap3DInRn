...
const privateRoomIdDict = [];
const iMei = '866947020186826';
//发起聊天-->建立聊天室
AppSocket.on(iMei, (data) => {
    if (data.action === "lunch_private_chat") {
        privateRoomIdDict[data.fromUid] = data.roomId;
...
AppSocket.on("private_chat_message_type", (data) => {
    if (data.typingUid !== iMei && data.fromUid === 'db04702856d9f69a') {
        let toUserTyping = data.actionType === "typing";
        console.log('typingiiiiiiiiiiiiiiiii:', toUserTyping);
    }
});